...
def generate_html(variants, sequences, output_path, template_dir=None):
    ...
    reference_info["fastaURL"] = Path(args.reference_fasta).name
    reference_info["indexURL"] = f"{Path(args.reference_fasta).name}.fai"
        ...
        'tracks': [
            {
                ...
                'type': 'variant',
                'format': 'vcf',
                'url': Path(vcf_path).name,
                'indexURL': f"{Path(vcf_path).name}.tbi"
            }
        ]
    }
